...
public class RM_MM_Stock_Report extends BaseActivity {
	/*
	 * Geographical Search
	*/
	...
	private ArrayList<HashMap<String, String>> getCFAtList;
	AlertDialog dialog;
	...
	*/
	...
	private TextView aplication,oe,segment,header;
	
	ArrayList<HashMap<String,Object>> arealist;
	ArrayList<HashMap<String, String>> segmentList;
	...
	int ParntNo;
	 ...
	 ArrayList<RM_MM_Stock_Model> partnoListFilter;
	 RadioButton rb_am_mechanic, rb_am_dealer ,rb_am_cfa;
 ...
 * Listing for all spinner
	*/
	...
	public void inti() {
		...
		super.inti();
		setContentView(R.layout.rm_mm_stock_report);
		...
		segmentList=new ArrayList<HashMap<String,String>>();
		...
		header.setText("Stock Report");

		
		mRGroupTwo=(RadioGroup) findViewById(R.id.rg_am2_trend);
		mRGroupTwo.setOnCheckedChangeListener(this);
		rb_am_mechanic=(RadioButton) findViewById(R.id.rb_am_mechanic);
		rb_am_dealer=(RadioButton) findViewById(R.id.rb_am_dealer);
		rb_am_cfa=(RadioButton) findViewById(R.id.rb_am_cfa);
		...
		statespinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				if(position!=0){
					  ...
					  mRGroupTwo.clearCheck();
				    /*	rb_am_mechanic.setChecked(false);
				    	rb_am_dealer.setChecked(false);
				    	rb_am_cfa.setChecked(false);*/
				        cityidd.clear();
						cityname.clear();
						amidd.clear();
						amname.clear();
						...
						if (dealerName!=null||dealerNameId!=null) {
							dealerName.clear();
							dealerNameId.clear();
						}
		...
		cityspinner.setOnItemSelectedListener(new OnItemSelectedListener() {
			...
			public void onItemSelected(AdapterView<?> arg0, View arg1,
				...
				if(position!=0){
					...
					getGeographyByCode(ContactType,Constants.GEOCODE_AREA_CUSTOMER,zoneid.get(zonespinner.getSelectedItemPosition()),ContactID,stateidd.get(statespinner.getSelectedItemPosition()),cityidd.get(cityspinner.getSelectedItemPosition()),"0","0");
					}
		...
		*/
		...
		spn_cfa_dealer_mg_spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
		 ...
		 if (ContactTypeAM.equalsIgnoreCase("4")) {
		      ...
		      getGeographyByCode(ContactType,Constants.GEOCODE_AREA_CUSTOMER,"0",ContactID,"0","0","0","0");
			}
					        ...
					        list_part_no.setAdapter(partNoAdapter);
					            ...
					            searchpartNo.addTextChangedListener(new TextWatcher() {
					    			...
					    			public void onTextChanged(CharSequence arg0, int arg1, int arg2, int arg3) {
					    				...
					    				if(RM_MM_Stock_Report.this.partNoAdapter!=null)
					    					RM_MM_Stock_Report.this.partNoAdapter.getFilter().filter(arg0);
					    			}
...
try {
//	Function GetStockReport_MIS(LocationType As String, ContactType As String, 
	//ContactID As String, CountryID As String, ZoneID As String, StateID As String, 
	//CityID As String, ByVal UserID As String, ByVal Password As String) As String
			...
			String ContactType=passworddetails.getString(Constants.CONTACTTYPEID, "");
			 if (ContactType.equalsIgnoreCase("4")) {
			      ContactType="RM";
			      request.put("ContactType",ContactType);
			   
				}
			 if (ContactType.equalsIgnoreCase("5")) {
			      ContactType="MM";
			      request.put("ContactType",ContactType);
				}
			 
			 if (ContactType.equalsIgnoreCase("27")) {
			      ContactType="CO";
			      request.put("ContactType",ContactType);
			    
				}
			 if (ContactType.equalsIgnoreCase("28")) {
			      ContactType="HO";
			      request.put("ContactType",ContactType);
			   
				}
			 
			 if (ContactType.equalsIgnoreCase("3")) {
			      ContactType="SI";
			      request.put("ContactType",ContactType);
				}
	...
	public void onCheckedChanged(RadioGroup group, int checkedId) {
		...
		super.onCheckedChanged(group, checkedId);
		 ...
		 if (checkedId==R.id.rb_am_cfa && rb_am_cfa.isChecked() ) {
		...
		else if (checkedId==R.id.rb_am_dealer && rb_am_dealer.isChecked() ) {
		getGeographyByCode(ContactType,Constants.GEOCODE_DLR,zoneid.get(zonespinner.getSelectedItemPosition()),ContactID,stateidd.get(statespinner.getSelectedItemPosition()),cityidd.get(cityspinner.getSelectedItemPosition()),"0",amidd.get(spnr_Am.getSelectedItemPosition()));
		stockLocation="D";
		if (partnoListFilter!=null) {
			partnoListFilter.clear();
		...
		else if (checkedId==R.id.rb_am_mechanic && rb_am_mechanic.isChecked()) {
			dealerName.clear();
			dealerNameId.clear();
			stockLocation="M";
            getMechanic();
		...
		GetDataFromNetwork dataFromNetwork = new GetDataFromNetwork(getCurrentContext(), ProgressDialog.STYLE_SPINNER,"Loading...", new GetDataCallBack() {
			...
			public void processResponse(Object response) {
				if(response!=null){
					 ...
					 segmentList = FetchingdataParser.getCFAObject(response.toString());
					if(segmentList!=null){
...
//
	...
	private void getCFA() {
		...
		GetDataFromNetwork dataFromNetwork = new GetDataFromNetwork(getCurrentContext(), ProgressDialog.STYLE_SPINNER,"Loading...", new GetDataCallBack() {
			...
			public void processResponse(Object response) {
				if(response!=null){
					 ...
					 getCFAtList = FetchingdataParser.getCFAObject(response.toString());
					if(getCFAtList!=null){
						SimpleAdapter segmentAdapter = new SimpleAdapter(getCurrentContext(),getCFAtList, android.R.layout.simple_spinner_item, new String[]{Constants.CODE}, new int[]{android.R.id.text1});
						segmentAdapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice);
						spn_cfa_dealer_mg_spinner.setAdapter(segmentAdapter);
		...
		LinkedHashMap<String,Object> request=new LinkedHashMap<String, Object>();
		SharedPreferences passworddetails=getSharedPreferences("PASSWORD", MODE_PRIVATE);
	//Function GetInternalContactCFA(ByVal CountryID As String, ByVal ZoneID As String, ByVal StateID As String, ByVal CityID As String, ByVal AMID As String, ByVal UserID As String, ByVal Password As String) As String
		...
		String webService =getResources().getString(R.string.Webservice_mis_android);
		String nameSpace=getResources().getString(R.string.nameSpace);
		String methodName="GetInternalContactCFA";
		String soapcomponent=getResources().getString(R.string.soapcomponent_androidmis);
		dataFromNetwork.setConfig(ipAddress, webService, nameSpace,methodName,soapcomponent);
		dataFromNetwork.sendData(request);
		dataFromNetwork.execute();
	...
	private void getGeographyByCode(final String contactType , final String geoName, final String geoId, final String ContactID, final String stateId, final String cityId, final String areaID, final String amId) {
		...
		SharedPreferences passworddetails=getSharedPreferences("PASSWORD", MODE_PRIVATE);
		try {
		//	Function GetGeographyByLogin_NEW(ByVal ContactType As String, ByVal GeoType As String, ByVal GeoID As String,
			//ByVal ContactID As String, ByVal StateID As String, ByVal CityID As String, ByVal AreaID As String,
			//ByVal AMID As String, ByVal UserID As String, ByVal Password As String) As String
