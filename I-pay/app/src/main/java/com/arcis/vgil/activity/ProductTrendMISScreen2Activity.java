...
public class ProductTrendMISScreen2Activity extends BaseActivity {
	...
	public  void inti()  {
		...
		setContentView(R.layout.product_trend_mis_screen2);
		...
		if(trendType.equals("Dealer")){
			...
			if(reportType!=1){
				...
				getRequestDataMap().put(Constants.ProductID, bundle.getString(Constants.ProductID));
			}else{
				txt8.setText("All Parts");
			}
		}else{
		...
		getRequestDataMap().put("IsMonthly", "1");
		...
		GetDataFromNetwork datafromnetwork = new GetDataFromNetwork(getCurrentContext(), ProgressDialog.STYLE_SPINNER, "Loading...", new GetDataCallBack() {
			...
			public void processResponse(Object result) {
				...
				if(result!=null){
					...
					int quantity = 0;
					double value = 0.0;
					ArrayList<HashMap<String, String>> monthelyList = FetchingdataParser.getProductMonthelyTrendList(result.toString());
					if(monthelyList!=null){

						String[] fromdealer = {Constants.MONTH,Constants.quantity,Constants.VALUE};

						int[] to = {R.id.position1,R.id.position2,R.id.position3};


						SimpleAdapter adapter = new SimpleAdapter(getCurrentContext(), monthelyList, R.layout.customer_trendlist_monthely_cell, fromdealer, to);
